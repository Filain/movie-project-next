...
const CharactersListComponent: FC<IProps> = ({char}) => {
    ...
    return (
        <div key={char.id} className={`${styles.wrap} ${theme}`}>
            {
                char.profile_path
                    ? <Image className={styles.image} src={`${poster}/${char.profile_path}`} alt={char.character}
                             width={100} height={100}/>
                    :
                    <Image className={styles.imageNot} src={'/images/notFound.svg'} alt={'Image not found '} width={80}
                           height={80}/>
            }
